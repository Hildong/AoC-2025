...
int main(int argc, char *argv[]) {
            ...
            for (long long i = firstId; i <= lastId; i++) {
                ...
                for (int divisor : divisors) {
                    if (divisor == strId.size()) {
                        bool allCharsAreSame =
                            std::all_of(strId.begin(), strId.end(), [&strId](char c) { return c == strId[0]; });
                        if (allCharsAreSame) {
    ...
    std::cout << "Total sum of all invalid ids with no restriction together is: " << invalidIdSumNoRestrictions
              << std::endl;
    return 0;
}
